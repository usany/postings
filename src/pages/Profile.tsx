...
// `
function Profile({ displayName, setDisplayName, isLoggedIn, userObj, setUserObj, value, setValue, side, setSide, sideNavigation, setSideNavigation, check, setCheck, counter, setCounter, setBottomNavigation }) {
  ...
  const onSubmit = async (event) => {
    ...
    if (newDisplayName === '') {
      ...
      //   }
      // });
      const usernameQuery = await supabase.from('practices').select('username')
      usernameQuery.data?.map((element) => {
        if (newDisplayName === element.username) {
          alert('중복 확인이 필요합니다')
          profileConfirmed = false
        }
      })
      ...
      //   console.log('error')
      // })
      await supabase.from('practices').upsert({ id: userObj.id, username: newDisplayName }).select()
      }
  ...
  return (  
    <div>
      ...
      <div className='flex justify-center'>제 유저 이름은 {displayName}</div>
      {attachment &&
        <div className='flex justify-center'>
