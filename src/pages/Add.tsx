...
function Add({ displayName, userObj, valuing }: {userObj: object, valuing: number}) {
...
//   const [counter, setCounter] = useState(0);
  ...
  useEffect(() => {
    if (cardNum) {
        console.log('sample')
        const getCardObject = async () => {
            const cardObjectBase = await supabase.from('cards').select().eq('id', cardNum)
            setCardDisplay(cardObjectBase.data?.map(element => element)[0])
            console.log(cardObjectBase.data?.map(element => element)[0])
        }
        getCardObject()
        setCardNum(null)
        // setInserted(false)
      ...
      if((locationInput !== '' || (locationOne !== '' && locationTwo !== '')) && from !== null && to !== null) {
        if (from.gmt > to.gmt) {
            ...
            const makeCard = async () => {
                ...
                .from('cards')
                .select('id')            
                const card = await supabase.from('cards').select('fromday')
                console.log(card)
                // console.log(valuing)
                // .upsert({ id: userObj.id})
                if (cards.data) {
                    const newId = (cards.data[cards.data.length-1].id+1)*5
                    await supabase
                    .from('cards')
                    .insert({
                        id: newId,
                        ...
                        fromclock: JSON.stringify(from),
                        toclock: JSON.stringify(to),
                        createdclock: Date.now()
                    })
                    setCardNum(newId)
                    // setInserted(true)
                }
...
//   console.log(display)
  ...
  const onChangeTo = (event) => {
    ...
    //     console.log(valuing)
    //     if (cards.data) {
    //         const newId = cards.data.length
    //         await supabase
    //         .from('cards')
    //         .upsert({
    //             id: newId,
    //             creatorid: userObj.id,
    //             creatorusername: users.data[0].username,
    //             round: 0,
    //             action: valuing
    //         })
    //     }
                        ...
                        :
                            <div>
                                ...
                                <div className='flex px-5 pt-5 pb-52'>
                                    <RegisteredCards msgObj={cardDisplay} isOwner={true} />
                                </div>
                            </div>
                        }
                    </div>
