...
function Navigation({ displayName, setScroll, isLoggedIn, userObj, setValue, check, setCheck, setMode }) {
  ...
  useEffect(() => {
    ...
    const checkUserPoints = async () => {
      const value = (await supabase
        .from('practices')
        .select()
        .eq('id', userObj.id)).data?.map((element) => element.points)[0]
      setPoints(value)
    }
    if (userObj) {
      setInterval(() => {
      ...
      {isLoggedIn &&
        ...
        >
          ...
          <h1 className='text-2xl	px-5 pt-5'>
            <Link to='/postings/profile' onClick={() => checkbox()}>{displayName}의 프로필</Link>
          </h1>
          <h1 className='text-2xl	px-5'>
            <Link to='/postings/ranking' onClick={() => checkbox()}>유저 랭킹</Link>
          </h1>
          <h1>
            <a className='text-2xl px-5' href='mailto:ckd_qja@naver.com' target="_blank">&emsp;신고하기</a>
          </h1>
          <h1 className='text-2xl	px-5'>
            <Link to='/postings/contact' onClick={() => checkbox()}>신고하기</Link>
          </h1>
          <h1 className='text-2xl px-5'>
            <a href='https://open.kakao.com/o/sT7ptgQd' target="_blank">&emsp;단체방</a>
          </h1>
          <h1 className='text-2xl px-5'>
